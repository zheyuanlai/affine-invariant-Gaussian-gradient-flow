...
### 5. `natural_gradient_nonconvex_instability` — nonconvex FR instability and BW stationarity
...
Fisher-Rao discretizations can produce very large FR gradients on this target.
The BW comparison is deliberately different: it uses the running minimum of the
BW gradient norm, matching the nonconvex stationarity theory, not a pointwise
Fisher-Rao norm. See
...
`wasserstein_bound_summary.csv`, `target_metadata.json`, and `run_metadata.json`.
Add `--smoke` for the fast reduced grid
(`outputs/natural_gradient_nonconvex_instability_smoke/`). The report assets are
regenerated by:
