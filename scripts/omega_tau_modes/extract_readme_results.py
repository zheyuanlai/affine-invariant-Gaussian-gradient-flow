...
Reads:
    outputs/omega_tau_modes/gaussian_grid/summary.csv
    outputs/omega_tau_modes/logconcave_grid/summary.csv
    outputs/omega_tau_modes/logconcave_grid/target_metadata.json
    outputs/omega_tau_modes/logconcave_grid/reference_optimum_meta.json
"""
import argparse
import json
import os
...
# repo root is three levels up: scripts/omega_tau_modes/<file>
...
def main():
    p = argparse.ArgumentParser(description="Print Markdown result tables from existing summaries.")
    p.add_argument("--gaussian-dir", default=os.path.join(ROOT, "outputs/omega_tau_modes/gaussian_grid"),
                   help="Directory with the Gaussian summary.csv "
                        "(legacy production data lives in outputs/gaussian_grid)")
    p.add_argument("--logconcave-dir", default=os.path.join(ROOT, "outputs/omega_tau_modes/logconcave_grid"),
                   help="Directory with the log-concave summary.csv + metadata "
                        "(legacy production data lives in outputs/logconcave_grid)")
    args = p.parse_args()

    g  = pd.read_csv(os.path.join(args.gaussian_dir, "summary.csv"))
    lc = pd.read_csv(os.path.join(args.logconcave_dir, "summary.csv"))
    with open(os.path.join(args.logconcave_dir, "target_metadata.json")) as f:
        meta = json.load(f)
    with open(os.path.join(args.logconcave_dir, "reference_optimum_meta.json")) as f:
        ref = json.load(f)
