...
Options:
    ...
    --outdir DIR     output directory   (default: outputs/omega_tau_modes/gaussian_grid)
"""
import argparse
import csv
import math
...
DEFAULT_CONFIG = os.path.join(
    os.path.dirname(__file__), "..", "..", "configs", "omega_tau_modes", "gaussian_target.yaml"
)
...
def parse_args():
    # Pre-parse --config / --smoke so the remaining defaults can come from the
    # chosen (and possibly smoke-overridden) config.
    pre = argparse.ArgumentParser(add_help=False)
    pre.add_argument("--config", default=DEFAULT_CONFIG,
                     help="YAML config (default: configs/omega_tau_modes/gaussian_target.yaml)")
    ...
    parser = argparse.ArgumentParser(parents=[pre], description="Run Gaussian gradient flow grid.")
    parser.add_argument("--dt",         type=float, default=cfg["defaults"]["dt"])
    parser.add_argument("--T",          type=float, default=cfg["defaults"]["T"])
    parser.add_argument("--n",          type=int,   nargs="+", default=cfg["dimensions"])
    parser.add_argument("--omega",      type=float, nargs="+", default=cfg["omega_grid"])
