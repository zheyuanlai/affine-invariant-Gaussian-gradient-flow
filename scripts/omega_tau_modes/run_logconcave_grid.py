...
from src.omega_tau_modes.targets          import LogCoshTarget
from src.omega_tau_modes.qmc_samples      import make_samples, push_forward
from src.omega_tau_modes.reference_optimum import load_or_compute
from src.omega_tau_modes.lc_dynamics      import logconcave_step
from src.omega_tau_modes.lc_initializations import get_logconcave_initialization, LC_INIT_NAMES
from src.omega_tau_modes.lc_metrics       import compute_lc_metrics, compute_lc_objective
from src.omega_tau_modes.utils            import spd_invsqrt, spd_sqrt, validate_params, make_q_vector
...
DEFAULT_CONFIG = os.path.join(
    os.path.dirname(__file__), "..", "..", "configs", "omega_tau_modes", "logconcave_target.yaml"
)
...
def _apply_smoke(cfg):
    ...
    for k, v in cfg.get("smoke", {}).items():
        if k == "defaults" and isinstance(v, dict):
            cfg.setdefault("defaults", {}).update(v)
        else:
            cfg[k] = v
    return cfg
...
def parse_args():
    ...
    pre.add_argument("--config", default=DEFAULT_CONFIG,
                     help="YAML config (default: configs/omega_tau_modes/logconcave_target.yaml)")
    pre.add_argument("--smoke", action="store_true",
                     help="Use the fast 'smoke' subset defined in the config")
    ...
    p = argparse.ArgumentParser(parents=[pre], description="Run log-concave gradient flow grid.")
