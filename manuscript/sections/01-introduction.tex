...
even, strongly convex, and uniformly Hessian-Lipschitz. A second construction
closes the fixed-step axis entirely: for every stepsize
$\Delta t\in(2/\kappa,2\kappa)$ an admissible target carries an exact nonoptimal
period-two orbit for both schemes (\Cref{thm:two-cycle}), so an order-one fixed
step does not converge at all, and the certified scale
$\Delta t=\Theta(\kappa^{-1})$ with its $\Theta(\kappa^2)$ iteration count is
intrinsic to fixed-step natural gradient rather than an artifact of the proofs
(\Cref{cor:fixed-step}). The covariance burn-in,
...
the affine invariance we exploit throughout. Closest to our deterministic theory,
\citet{kumar2025squareroot} prove convergence guarantees for Gaussian
natural-gradient variational inference, in discrete time and for its
continuous-time flow, through a square-root covariance parameterization; for
stochastic natural-gradient variational inference, \citet{wu2024understanding}
obtain the first $O(1/T)$ non-asymptotic rate in the conjugate-likelihood case.
Our analysis differs from these and from a generic
natural-gradient convergence argument in that it uses the exact Gaussian
covariance equation in the intrinsic Fisher--Rao geometry rather than a
smoothness surrogate or a chosen parameterization, and keeps every constant
in optimizer-whitened, affine-invariant form; this is what yields the
three-stage decomposition, the optimizer-whitened condition number, and the
matching lower bounds at every stage.
...
log-concavity. \citet{kim2026price} analyze stochastic Gaussian variational
inference with Price's Hessian-based gradient estimator across the
Bures--Wasserstein and parameter-space geometries, an estimator-level overlap
with our joint Price/Hessian oracle, and \citet{luu2024variance} add
control-variate variance reduction on the Bures--Wasserstein manifold; neither
treats the Fisher--Rao geometry or its variance floors. We use the Bures--Wasserstein flow in two limited roles, as a
