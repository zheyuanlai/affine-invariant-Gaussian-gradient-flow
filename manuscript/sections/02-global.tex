...
The rate produced by \eqref{eq:FR-W2} and \Cref{lem:cov-cont} still carries the affine
...
the squared Riemannian distance at the current iterate $a_n=(m_n,C_n)$ as
...
\end{equation}
which is the distance of the metric \eqref{eq:FR-metric}. As the entropy term $\calE_1$ is
...
\begin{equation*}
    ...
    \argmin_{a\in\Aspace}
    \left\{
        \calE_2(a_n)+\inner{\grad\calE_2(a_n)}{\Log_{a_n}(a)}+\frac1{2\Delta t}d(a,a_n)^2
    \right\},
\end{equation*}
which gives
\begin{equation*}
...
\begin{proposition}\label{prop:upd-map-Riem}
    ...
    \end{equation*}
    Then the discrete algorithm with Riemannian distance \eqref{upd:Riem} gives the one-step
    update
    \begin{equation*}
...
The proof is given in \Cref{app:det-proofs}.
...
The discrete algorithm with Riemannian distance \eqref{upd:Riem} is geometrically natural but
...
\end{equation}
Thus \eqref{upd:KL} is a forward--backward mirror step in the expectation parameters, which
linearizes the potential part $\calE_2$ explicitly and treats the entropy $\calE_1$
implicitly; only one linear solve is required per iteration. The step is the natural one for
...
\begin{proof}
    ...
    $z_n:=\lambda_{\min}(C_n^{-1})/\alpha$, so $z_n=1+(z_0-1)(1+\Delta t)^{-n}$; if $z_0\geq1$
    then $z_n\geq1$ for all $n$, and if $z_0<1$ then $z_n\geq\tfrac12$ as soon as
...
The proof is given in \Cref{app:det-proofs}. The relevant descent divergence is the
...
\begin{remark}[Comparing the two schemes]
...
\begin{corollary}[Sharpness of the global-localization prefactor]\label{cor:sharp-disc}
    For the globally fixed-step schemes \eqref{upd:Riem} and \eqref{upd:KL}, the factor
    $\kappa/\Delta t$ in \Cref{thm:glob-Riem,thm:glob-KL} is sharp, up to constants independent
    of $\kappa$ for each fixed $\gamma=\kappa\Delta t$, for a constant-factor reduction of the
    energy. In particular, with $\Delta t=\Theta(\kappa^{-1})$, both methods have worst-case
    global-localization complexity $\Theta(\kappa^2)$ in the condition-number exponent, and
    this remains true under the additional assumption $\norm{V'''}_\infty=O(1)$.
\end{corollary}
...
\begin{remark}[Limitations of the lower bound]
    The theorem proves sharpness of the polynomial condition-number dependence for explicit
    methods committed to one globally fixed step. It does not rule out an $O(\kappa)$
    global-localization result for an adaptive relative-curvature step, an Armijo or
    backtracking line search, or an implicit or Rosenbrock-stabilized mean update. Indeed the
    construction has $c_nA(m_n,c_n)\simeq1$, so a method whose stability test is based on the
    current relative curvature could accept a step of order one and avoid this particular
    slowdown.
\end{remark}
...
\begin{lemma}[Wasserstein covariance bootstrap]\label{lem:cov-BW}
    ...
    $0<\eta\leq1/\beta$, the Bures--Wasserstein forward--backward step
...
\begin{remark}[What the warm starts do not remove]
...
%   §2.3
%     natural-gradient.tex l.308-362 (splitting, Riemannian distance, forward/backward steps,
%       boxed update; kept near-verbatim) -> eq:Riem-dist, upd:Riem
%     natural-gradient.tex l.366-397 (prop:upd-map-Riem) -> prop:upd-map-Riem
%     improved-global-local.tex l.1845-1847 ("exponential retraction") -> retraction remark
...
%     improved-global-local-stoch.tex l.3994-4025 (lem:KL-argmin) -> forward-backward mirror sentence
