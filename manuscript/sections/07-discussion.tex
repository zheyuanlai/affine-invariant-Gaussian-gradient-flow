...
of the fixed-step schemes is sharp for a constant-factor reduction, the
two-cycle shows that no order-one fixed stepsize converges at all, and the
...
itself sharp. The fixed-step axis is closed: the bump train and the two-cycle
together show that no fixed stepsize improves on the $\Theta(\kappa^2)$
worst case, and that every order-one fixed step fails outright. Whether an
adaptive, line-search, or implicit scheme attains $O(\kappa)$ remains open; a
stability test based on the relative curvature $c_nA_n$ does not suffice, since
the two-cycle holds it at one exactly, while a test based on the secant slope
$b(m_n,c_n)/m_n$, or a monotone line search that rejects the non-decreasing
flip, is not ruled out. A closed-form discrete local
