...
The Riemannian scheme requires a matrix exponential at every step. The KL resolvent scheme \eqref{upd:KL} of \Cref{sec:global} avoids it, but its rational form does not extend safely to the whole family: the natural general-metric analogue multiplies the resolvent by a scalar prefactor that is not sign-definite for $\tau < 0$, so the resulting update need not remain in the SPD cone for admissible metrics. We therefore state the general-metric extension only for the retraction scheme \eqref{upd:affine-Riem}, whose covariance update is SPD by construction; at the Fisher--Rao point $(\eta, \omega, \tau) = (1, \frac{1}{2}, 0)$ the resolvent scheme \eqref{upd:KL} of \Cref{sec:global} remains available with its full convergence theory.
...
%     first-order-consistency / not-a-proximal-update caveat) -> NOT CARRIED
%     [DEFECT: the general (eta,omega,tau) KL resolvent update in the source has a scalar
%      prefactor 1 + Dt(omega - tau Tr(C_n E[Hess log rho_post]))/(2 omega(omega + N_theta tau))
%      that is negative for admissible tau < 0 (e.g. N_theta=2, eta=omega=1, tau=-49/100,
%      C_n=2I, Dt=1/10 gives C_{n+1} = -28/11 I, not SPD). No positivity condition is proved
%      in any source, so the general KL display is replaced by a scope sentence; only the
%      SPD-safe retraction scheme upd:affine-Riem is stated for the general family.]
