...
| 5.4 general discretizations | the $(\eta,\omega,\tau)$ Riemannian scheme only, recovering `upd:Riem` at $(1,\tfrac12,0)$. **Scope revised (2026-07):** the general KL-type variant is NOT carried -- its resolvent's scalar prefactor is negative for admissible $\tau<0$ (leaves the SPD cone; no positivity condition proved in any source), so it is replaced by a scope sentence; `upd:KL` remains available at the Fisher--Rao point | `natural-gradient.tex` `upd:affine-invariant-Riem`; KL variant (l.2875-2882) dropped, see DEFECT note in `05-geometry.tex` SOURCES block |
