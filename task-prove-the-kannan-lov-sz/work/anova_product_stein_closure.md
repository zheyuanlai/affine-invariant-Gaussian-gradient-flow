# Affine--orthogonal Gaussian ANOVA for product log-concave inputs
...
\[
 ...
 \le 900\,E R^2. \tag{1}
\]
The constant is independent of \(n\) and of the factors.

The proof is a tensorized calibration of the unresolved full
multidimensional ANOVA gate; it uses the diagonal one-dimensional Stein
kernels and does not apply to an irreducible law.

## 1. Affine reduction and Gaussian jets

Subtract the \(L^2(\nu)\)-orthogonal projection of \(f\) onto
\(\operatorname{Aff}\). For an affine \(\ell(S)=c+a\cdot S\),
\[
 E[\ell(S)\mid X]+E[\ell(S)\mid G]=\ell(S),
\]
so \(R\) is unchanged. We may therefore assume
\(f\perp\operatorname{Aff}\); then
\[
...
\]
Define the order-\(k\) aggregate energies
\[
...
For each factor let \(\tau_i\) be its canonical Stein kernel. The one-dimensional
...
\[
 \|f\|_2^2
 =V_0+M_1+V_1+\sum_{k\ge2}(V_k+M_k)
 \le12V_1+ER^2+2V_1+800ER^2
 \le815ER^2. \tag{14}
\]
Because \(f\perp\operatorname{Aff}\), \(\|f\|_2^2\) is exactly its squared
distance to the affine space. This proves (1) with the rounded constant 900.
...
inequality to the approximants and pass first at a fixed jet order, then by
